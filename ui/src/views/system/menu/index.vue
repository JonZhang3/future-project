...
import { SystemMenuType, CommonStatus } from '@/utils/constants'
...
const submitForm = async () => {
  ...
  try {
    if (
      menuForm.value.type === SystemMenuType.DIR ||
      menuForm.value.type === SystemMenuType.MENU
    ) {
...
<template>
  ...
  <ContentWrap>
    ...
    <el-table v-loading="loading" table-layout="auto" row-key="id" :data="menuData"
      ...
      <el-table-column label="操作">
        <template #default="scope">
          <el-button link type="primary" v-hasPermi="['system:menu:update']" @click="handleUpdate(scope.row)">
            <Icon icon="ep:edit" class="mr-1px" /> {{ t('action.edit') }}
          </el-button>
          <el-button link type="primary" v-hasPermi="['system:menu:delete']" @click="handleDelete(scope.row)">
  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle" maxHeight="400px" width="40%">
    <el-form :model="menuForm" :rules="rules" :inline="true" label-width="120px" label-position="right">
      <el-row :gutter="24">
        <el-col :span="24">
          <el-form-item label="上级菜单">
            <el-tree-select node-key="id" v-model="menuForm.parentId" :props="menuProps" :data="menuData"
              check-strictly />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="菜单类型" prop="type">
            <el-radio-group v-model="menuForm.type">
              <el-radio-button v-for="dict in getIntDictOptions(DICT_TYPE.SYSTEM_MENU_TYPE)" :key="dict.value"
                :label="dict.value">
                {{ dict.label }}
              </el-radio-button>
            </el-radio-group>
        ...
        <template v-if="menuForm.type !== 3">
          ...
          <el-col :span="12">
            <el-form-item label="路由地址" prop="path">
              <template #label>
                <Tooltip titel="路由地址" message="访问的路由地址，如：`user`。如需外网地址时，则以 `http(s)://` 开头" />
              </template>
        ...
        <el-col :span="12">
          <el-form-item label="菜单状态" prop="status">
            <el-radio-group v-model="menuForm.status">
              <el-radio-button v-for="dict in getIntDictOptions(DICT_TYPE.COMMON_STATUS)" :key="dict.value"
                :label="dict.value">
                {{ dict.label }}
              </el-radio-button>
            </el-radio-group>
        ...
        <template v-if="menuForm.type === 2">
          <el-col :span="12">
            <el-form-item label="缓存状态" prop="keepAlive">
              <template #label>
                <Tooltip titel="缓存状态" message="选择缓存时，则会被 `keep-alive` 缓存，需要匹配组件的 `name` 和路由地址保持一致" />
              </template>
    ...
    <!-- 操作按钮 -->
    <template #footer>
      <el-button v-if="['create', 'update'].includes(actionType)" type="primary" :loading="actionLoading"
        @click="submitForm">
        {{ t('action.save') }}
      </el-button>
