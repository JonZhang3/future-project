...
const handleStatusChange = async (row: UserVO) => {
  const text = row.status === CommonStatus.ENABLE ? '启用' : '停用'
  message
    .confirm('确认要"' + text + '""' + row.username + '"用户吗?', t('common.reminder'))
    .then(async () => {
      row.status =
        row.status === CommonStatus.ENABLE ? CommonStatus.ENABLE : CommonStatus.DISABLE
    ...
    .catch(() => {
      row.status =
        row.status === CommonStatus.ENABLE ? CommonStatus.DISABLE : CommonStatus.ENABLE
    })
}
// 重置密码
...
<template>
  <div class="flex">
    ...
    <el-card class="w-4/5 user" style="margin-left: 10px" :gutter="12" shadow="hover">
      ...
      <Search :schema="allSchemas.searchSchema" @search="setSearchParams" @reset="setSearchParams" ref="searchForm" />
      <!-- 操作工具栏 -->
      <div class="mb-10px">
        <el-button type="primary" v-hasPermi="['system:user:create']" @click="handleAdd">
          <Icon icon="ep:zoom-in" class="mr-5px" /> {{ t('action.add') }}
        </el-button>
        <el-button type="info" v-hasPermi="['system:user:import']" @click="importDialogVisible = true">
          <Icon icon="ep:upload" class="mr-5px" /> {{ t('action.import') }}
        </el-button>
        <el-button type="warning" v-hasPermi="['system:user:export']" @click="exportList('用户数据.xls')">
          <Icon icon="ep:download" class="mr-5px" /> {{ t('action.export') }}
        </el-button>
      </div>
      <!-- 列表 -->
      <Table :columns="allSchemas.tableColumns" :selection="false" :data="tableObject.tableList"
        :loading="tableObject.loading" :pagination="{
          total: tableObject.total
        }" v-model:pageSize="tableObject.pageSize" v-model:currentPage="tableObject.currentPage" @register="register">
        <template #sex="{ row }">
        ...
        <template #action="{ row }">
          <el-button link type="primary" v-hasPermi="['system:user:update']" @click="handleUpdate(row)">
            <Icon icon="ep:edit" class="mr-1px" /> {{ t('action.edit') }}
          </el-button>
          <el-button link type="primary" v-hasPermi="['system:user:update']" @click="handleDetail(row)">
            <Icon icon="ep:view" class="mr-1px" /> {{ t('action.detail') }}
          </el-button>
          <el-button link type="primary" v-hasPermi="['system:user:update-password']" @click="handleResetPwd(row)">
            <Icon icon="ep:key" class="mr-1px" /> 重置密码
          </el-button>
          <el-button link type="primary" v-hasPermi="['system:permission:assign-user-role']" @click="handleRole(row)">
            <Icon icon="ep:key" class="mr-1px" /> 分配角色
          </el-button>
          <el-button link type="primary" v-hasPermi="['system:user:delete']" @click="delList(row.id, false)">
  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle">
    ...
    <Form v-if="['create', 'update'].includes(actionType)" :rules="rules" :schema="allSchemas.formSchema" ref="formRef">
      <template #deptId>
        <el-tree-select node-key="id" v-model="deptId" :props="defaultProps" :data="deptOptions" check-strictly />
      </template>
      <template #postIds>
    ...
    <!-- 对话框(详情) -->
    <Descriptions v-if="actionType === 'detail'" :schema="allSchemas.detailSchema" :data="detailRef">
      <template #deptId="{ row }">
    ...
    <!-- 操作按钮 -->
    <template #footer>
      <el-button v-if="['create', 'update'].includes(actionType)" type="primary" :loading="loading" @click="submitForm">
        {{ t('action.save') }}
      </el-button>
  ...
  <!-- 导入 -->
  <Dialog v-model="importDialogVisible" :title="importDialogTitle" :destroy-on-close="true" maxHeight="350px">
