...
const handleScope = async (type: string, row: RoleVO) => {
  ...
  if (type === 'menu') {
    ...
    const deptRes = await listSimpleDept()
...
<template>
  ...
  <ContentWrap>
    ...
    <Table :columns="allSchemas.tableColumns" :selection="false" :data="tableObject.tableList"
      ...
      <template #action="{ row }">
        <el-button link type="primary" v-hasPermi="['system:role:update']" @click="handleUpdate(row)">
          <Icon icon="ep:edit" class="mr-1px" /> {{ t('action.edit') }}
        </el-button>
        <el-button link type="primary" v-hasPermi="['system:role:update']" @click="handleDetail(row)">
          <Icon icon="ep:view" class="mr-1px" /> {{ t('action.detail') }}
        </el-button>
        <el-button link type="primary" v-hasPermi="['system:permission:assign-role-menu']"
          @click="handleScope('menu', row)">
          <Icon icon="ep:basketball" class="mr-1px" /> 菜单权限
        </el-button>
        <el-button link type="primary" v-hasPermi="['system:permission:assign-role-data-scope']"
          @click="handleScope('data', row)">
          <Icon icon="ep:coin" class="mr-1px" /> 数据权限
        </el-button>
        <el-button link type="primary" v-hasPermi="['system:role:delete']" @click="delList(row.id, false)">
  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle">
    <!-- 对话框(添加 / 修改) -->
    <Form v-if="['create', 'update'].includes(actionType)" :schema="allSchemas.formSchema" :rules="rules"
      ref="formRef" />
    <!-- 对话框(详情) -->
      ...
      ">
        <el-card class="box-card">
          <template #header>
            ...
            全选/全不选:
            <el-switch v-model="treeNodeAll" inline-prompt active-text="是" inactive-text="否"
              @change="handleCheckedTreeNodeAll()" />
          </template>
          <el-tree ref="treeRef" node-key="id" show-checkbox :default-checked-keys="defaultCheckedKeys"
            :check-strictly="!checkStrictly" :props="defaultProps" :data="treeOptions" empty-text="加载中，请稍后" />
        </el-card>
      </el-form-item>
    </el-form>
    <!-- 操作按钮 -->
