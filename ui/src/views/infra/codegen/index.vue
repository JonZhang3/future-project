...
const handleSynchDb = (row: CodegenTableVO) => {
    ...
    message.confirm('确认要强制同步' + tableName + '表结构吗?', t('common.reminder')).then(async () => {
...
<template>
    ...
    <ContentWrap>
        ...
        >
            ...
            <template #action="{ row }">
                <el-button link type="primary" v-hasPermi="['infra:codegen:preview']" @click="handlePreview(row)">
                    <Icon icon="ep:view" class="mr-1px" /> {{ t('action.preview') }}
                </el-button>
                <el-button link type="primary" v-hasPermi="['infra:codegen:update']" @click="handleEditTable(row)">
                    <Icon icon="ep:edit" class="mr-1px" /> {{ t('action.edit') }}
                </el-button>
                <el-button link type="primary" v-hasPermi="['infra:codegen:delete']" @click="delList(row.id, false)">
                    <Icon icon="ep:delete" class="mr-1px" /> {{ t('action.del') }}
                </el-button>
                <el-button link type="primary" v-hasPermi="['infra:codegen:update']" @click="handleSynchDb(row)">
                    <Icon icon="ep:refresh" class="mr-1px" /> {{ t('action.sync') }}
                </el-button>
                <el-button link type="primary" v-hasPermi="['infra:codegen:download']" @click="handleGenTable(row)">
